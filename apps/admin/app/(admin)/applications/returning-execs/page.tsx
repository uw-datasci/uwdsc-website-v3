...
export default function ReturningExecsPage() {
  ...
  const [updatingSelectionId, setUpdatingSelectionId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        setLoading(true);
        setError(null);
        const { submissions: list, positionReview: pr } =
          await getAllReturningExecs();
        setSubmissions(list);
  ...
  const handleSelectionStatusChange = async (
    ...
    const selection = selectedSubmission.position_selections.find(
      (s) => s.id === selectionId,
    );
