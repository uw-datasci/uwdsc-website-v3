...
/**
 * PATCH /api/applications/review/questions/[id]
 * Update a scoped question.
 */
export const PATCH = withVpAccess<Params>(
  async (request, { params }, _user, scope) => {
    try {
      ...
      if (!updated)
        return ApiResponse.badRequest("Question not found", "Not found");
...
/**
 * DELETE /api/applications/review/questions/[id]
 * Delete a scoped question.
 */
export const DELETE = withVpAccess<Params>(
  async (_request, { params }, _user, scope) => {
    try {
      ...
      const deleted = await applicationService.deleteQuestion(
        scope,
        relationId,
      );
      if (!deleted)
