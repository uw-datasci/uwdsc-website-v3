...
Migrations live in `packages/server/db`. Use the root scripts:

```bash
# From repo root: create new migration
...
Use the Postgres connection from `@uwdsc/db` (see [Database Setup](/guides/database)). Run seed scripts from the app or a script that loads env and uses the db package.
