  ...
  return (
    ...
    >
      <div className="relative mt-6 w-full max-w-80 flex-1">
        ...
        {middle ? (
          <Polaroid fact={middle} centerX="65%" centerY="47%" rotate="12.44deg" />
        ) : null}
        {front ? (
          <Polaroid fact={front} centerX="35%" centerY="74%" rotate="-13.76deg" />
        ) : null}
      </div>
    </div>
  );
...
function Polaroid({ fact, centerX, centerY, rotate }: PolaroidProps) {
  return (
    ...
    >
      ...
      >
        {fact.visual.icon ? (
          <Image src={fact.visual.icon} alt="" fill className="object-cover" />
        ) : null}
      </div>
      <div className="mt-2 flex flex-col items-center gap-0.5">
        <p
          className={cn(
            mono,
            "text-center text-[0.625rem] leading-tight whitespace-nowrap text-black"
          )}
        >
          {fact.label}
        </p>
        <p
          className={cn(
            display,
            "text-center text-[0.9375rem] leading-none whitespace-nowrap text-black"
          )}
        >
          {fact.value}
        </p>
