...
interface PasswordResetsSlideProps {
  readonly slide: PasswordResetsSlideData;
}

/**
 * "Locked out # times" slide: a big centered stat number with the same
 * decorative blob shapes used in {@link MinutesOnSiteSlideData}, piled along
 * the bottom of the slide instead of ringing the number.
 *
 * Data injection points (see {@link PasswordResetsSlideData}):
 * - `slide.eyebrow`: small label above the heading, e.g. "Oops".
 * - `slide.heading`: headline above the stat, e.g. "LOCKED OUT".
 ...
 * - Blob shapes and colors: `MushroomBlob`, `FlowerBlob`, `LeafBlob` below,
 *   each a hardcoded multi-color SVG. Change their `fill` values or swap in
 *   a different shape.
 * - Blob layout: the `pileOrnaments` array controls which shape, position,
 *   size and rotation each pile item uses.
 */
export function PasswordResetsSlide({ slide }: PasswordResetsSlideProps) {
  const pileOrnaments: {
    readonly kind: "mushroom" | "flower" | "leaf";
  ...
  return (
    <div
      className={cn(
        "relative flex h-full w-full flex-col items-center justify-center overflow-hidden bg-[#ff8f64] px-4 py-5 text-black sm:px-8 sm:py-6",
        tiltWarp.variable,
    ...
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-[38%] overflow-visible"
      >
        {pileOrnaments.map((ornament, index) => {
          const Blob = PILE_BLOBS[ornament.kind];
          return (
            <div
              key={index}
      ...
      <div className="relative z-10 flex h-full min-h-0 w-full max-w-104 flex-col items-center justify-center gap-4 pt-6 pb-2 sm:max-w-96 sm:gap-6 sm:pt-10 sm:pb-2">
        <div className="text-center">
          <p
            className={cn(
              display,
              "text-[1.1rem] leading-none tracking-tight text-black sm:text-[1.3rem]"
            )}
          >
            {slide.eyebrow}
          </p>
          <h2
            className={cn(
              display,
              "mt-1 text-[1.7rem] leading-none tracking-tight text-white sm:mt-1 sm:text-[1.9rem]"
            )}
          >
            {slide.heading}
          </h2>
