...
class EmailService {
  ...
  constructor() {
    ...
    if (!key) throw new ApiError("RESEND_API_KEY is not set", 500, "Configuration error");

    this.resend = new Resend(key);
    this.from =
      process.env.RESEND_FROM_EMAIL ?? "UW Data Science Club <noreply@mail.uwdatascience.ca>";
